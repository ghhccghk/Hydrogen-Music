    ...
    <div class="list-item" :class="{'list-item-selected': (item.id == router.currentRoute.value.fullPath.split('/')[3] && listType2 != 2) || (otherStore.currentVideoId == item.vid && listType2 == 2)}" v-for="(item, index) in libraryList" @click="showDetail(index, item)" @contextmenu="openMenu($event,item)">
        ...
        <div class="item-other">
            ...
            <div class="item-info">
              ...
              <div v-if="listType1 == 1 && listType2 == 3" class="item-artist">
                <span class="artist">{{ item.dj?.nickname }}</span>
              </div>
              <span v-if="listType1 == 1 && listType2 == 3" class="item-size">{{ item.programCount || 0 }}期</span>
              <span v-if="!(listType1 == 1 && listType2 == 1) && !(listType1 == 1 && listType2 == 2) && !(listType1 == 1 && listType2 == 3)"
                    class="item-size">{{ (item.trackCount ?? item.size) }}首</span>
            </div>
        </div>
    </div>
  </div>
