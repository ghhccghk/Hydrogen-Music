...
import LibrarySongList from '../library/LibrarySongList.vue'
...
const onPlayAll = () => {
  ...
  const ensureAll = async () => {
    ...
    while (!finished.value) {
      ...
      if (radioInfo.value?.programCount && (programSongs.value.length + list.length) >= radioInfo.value.programCount) {
        finished.value = true;
        break
      }
    }
  ...
  <div class="library-detail">
    <div class="view-control">
      <svg class="router-last" height="200" p-id="1053" t="1669039513804" version="1.1"
           viewBox="0 0 1024 1024" width="200" xmlns="http://www.w3.org/2000/svg" @click="router.back()">
    ...
    <div class="library-introduce">
      ...
      <transition name="metro" @after-enter="onAfterEnter" @after-leave="onAfterLeave">
        <div v-if="introduceDetailShow" :class="{'introduce-detail-text-active': introduceDetailShowDelay}"
             class="introduce-detail-text">
          <div class="detail-text">
            <p class="text">{{ radioDesc }}</p>
          </div>
          <div class="text-close" @click="introduceDetailShow = false">
            <svg class="icon" height="200" p-id="1965" t="1671966797621" version="1.1"
    ...
    <div :class="{'library-content3': true}" class="library-content">
      <div v-if="loading" class="status">正在加载...</div>
      <div v-else-if="error" class="status">{{ error }}</div>
      <LibrarySongList v-else :key="`dj-${rid}`" :songlist="programSongs"/>
    </div>
  </div>
</template>
